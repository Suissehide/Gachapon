/**
 * Définitions de l'équipement — pools de stats, barèmes par rareté et noms de
 * pièces.
 *
 ...
 * `prisma/seed/equipment.ts` n'y garde que l'écriture en base.
 */
  ...
  for (const setKey of SETS) {
    for (const slot of SLOTS) {
      ...
      for (const stat of pool) {
        // MAIN_STAT_SCALE est clé en `string` (il ne couvre pas toutes les
        // stats d'`EquipmentBonuses` : ni `spdPct`, ni `lifestealPct`). La
        // garde attrape une divergence avec SLOT_MAIN_STATS, qui produirait
        // sinon une pièce à bonus `undefined`.
        const scale = MAIN_STAT_SCALE[stat]
        if (!scale) {
          throw new Error(
            `Aucun barème de stat principale pour « ${stat} » : SLOT_MAIN_STATS et MAIN_STAT_SCALE ont divergé.`,
          )
        }
