/**
 * Définitions des tours élémentaires — difficulté par étage, sprites, butin
 * et libellés.
 *
 ...
 * `prisma/seed/tower.ts` n'y garde que l'écriture en base.
 */
...
/**
 * Lecture d'une courbe indexée par étage.
 *
 * `assertFloorInRange` garde bien l'exécution, mais `noUncheckedIndexedAccess`
 * ne relie pas cette garde à l'index qui suit : le type reste
 * `number | undefined`. Ce lecteur porte les deux, et une courbe trop courte
 * lève au lieu de propager un `NaN` dans un étage seedé.
 */
function scaleAt(
  ...
  const value = table[floor - 1]
  if (value === undefined) {
    throw new Error(`${what} : aucune valeur pour l'étage ${floor}`)
  }
  return value
}

export function towerEnemyPower(floor: number) {
  return towerEnemyStatsAtScale(
    scaleAt(FLOOR_SCALE, floor, 'Échelle de difficulté'),
    floor,
  )
}
...
export function towerFloorLoot(floor: number) {
  const s = scaleAt(LOOT_SCALE, floor, 'Échelle de butin')
  return {
    firstClear: {
