...
export interface IUpgradeRepository {
  ...
  bulkUpdateConfigs(upgrades: { type: string; level: number; effect: number; dustCost: number }[]): Promise<void>
  bulkUpdateConfigsInTx(tx: PrimaTransactionClient, upgrades: { type: string; level: number; effect: number; dustCost: number }[]): Promise<void>
  upsertUserUpgradeInTx(tx: PrimaTransactionClient, userId: string, type: string, level: number): Promise<UserUpgrade>
  decrementUserDustInTx(tx: PrimaTransactionClient, userId: string, amount: number): Promise<{ newDustTotal: number }>
}
