...
export const adminAchievementsRouter: FastifyPluginAsyncZod = async (fastify) => {
  ...
  fastify.post('/', { onRequest: auth, schema: { body: achievementSchema } }, async (request, reply) => {
  ...
  fastify.patch(
    '/:id',
    { onRequest: auth, schema: { params: z.object({ id: z.string().uuid() }), body: achievementSchema.partial() } },
    async (request) => {
