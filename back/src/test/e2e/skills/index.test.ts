...
describe('Skills routes', () => {
  ...
  beforeAll(async () => {
    ...
    const flux = await prisma.skillBranch.create({
      data: { nameFr: 'Flux', nameEn: 'Flux', descriptionFr: 'Tokens & Énergie', descriptionEn: 'Tokens & Énergie', icon: 'Zap', color: '#6c47ff', order: 1 },
    })
    const fortune = await prisma.skillBranch.create({
      data: { nameFr: 'Fortune', nameEn: 'Fortune', descriptionFr: 'Gacha & Chance', descriptionEn: 'Gacha & Chance', icon: 'Sparkles', color: '#f59e0b', order: 2 },
    })
    const collection = await prisma.skillBranch.create({
      data: { nameFr: 'Collection', nameEn: 'Collection', descriptionFr: 'Dust & Boutique', descriptionEn: 'Dust & Boutique', icon: 'Gem', color: '#10b981', order: 3 },
    })

    const regen = await prisma.skillNode.create({
      data: {
        branchId: flux.id, nameFr: 'Regen', nameEn: 'Regen', descriptionFr: 'Réduit le délai de régénération des tokens', descriptionEn: 'Réduit le délai de régénération des tokens',
    ...
    const multiToken = await prisma.skillNode.create({
      data: {
        branchId: flux.id, nameFr: 'Multi-token', nameEn: 'Multi-token', descriptionFr: 'Chance multi-token', descriptionEn: 'Chance multi-token',
    ...
    const luck = await prisma.skillNode.create({
      data: {
        branchId: fortune.id, nameFr: 'Luck', nameEn: 'Luck', descriptionFr: 'Augmente les taux de drop', descriptionEn: 'Augmente les taux de drop',
    ...
    const bouleDor = await prisma.skillNode.create({
      data: {
        branchId: fortune.id, nameFr: "Boule d'or", nameEn: "Boule d'or", descriptionFr: "Chance d'obtenir une boule en or", descriptionEn: "Chance d'obtenir une boule en or",
    ...
    const dustPlus = await prisma.skillNode.create({
      data: {
        branchId: collection.id, nameFr: 'Dust+', nameEn: 'Dust+', descriptionFr: 'Plus de dust lors du recyclage', descriptionEn: 'Plus de dust lors du recyclage',
    ...
    const reduction = await prisma.skillNode.create({
      data: {
        branchId: collection.id, nameFr: 'Réduction', nameEn: 'Réduction', descriptionFr: 'Réduit les prix en boutique', descriptionEn: 'Réduit les prix en boutique',
