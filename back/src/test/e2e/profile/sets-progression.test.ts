...
describe('GET /users/:username/profile/sets-progression', () => {
  ...
  beforeAll(async () => {
    ...
    const setA = await prisma.cardSet.create({ data: { nameFr: `SetA${suffix}`, nameEn: `SetA${suffix}`, isActive: true, hue: 35 } })
    const setB = await prisma.cardSet.create({ data: { nameFr: `SetB${suffix}`, nameEn: `SetB${suffix}`, isActive: true } })
    const a1 = await prisma.card.create({ data: { nameFr: 'a1', nameEn: 'a1', rarity: 'COMMON', dropWeight: 10, setId: setA.id } })
    const a2 = await prisma.card.create({ data: { nameFr: 'a2', nameEn: 'a2', rarity: 'COMMON', dropWeight: 10, setId: setA.id } })
    await prisma.card.create({ data: { nameFr: 'a3', nameEn: 'a3', rarity: 'COMMON', dropWeight: 10, setId: setA.id } })
    await prisma.card.create({ data: { nameFr: 'b1', nameEn: 'b1', rarity: 'COMMON', dropWeight: 10, setId: setB.id } })
    await prisma.card.create({ data: { nameFr: 'b2', nameEn: 'b2', rarity: 'COMMON', dropWeight: 10, setId: setB.id } })
