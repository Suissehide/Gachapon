...
export const fetchWithAuth = async (
  ...
  // Recalculé à chaque appel de `makeRequest` (donc à chaque tentative, y
  // compris le retry après refresh) via `withAcceptLanguage` — voir sa
  // JSDoc (`i18n/index.ts`) pour le piège de mémorisation qu'elle évite.
  const makeRequest = () =>
    fetch(input, {
      ...init,
      headers: withAcceptLanguage(init?.headers),
      credentials: 'include',
    })
