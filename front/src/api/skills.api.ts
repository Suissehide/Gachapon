...
export const SkillsApi = {
  ...
  invest: async (
    nodeId: string,
  ): Promise<{ nodeId: string; newLevel: number; skillPoints: number }> => {
  ...
  investBatch: async (
    allocations: { nodeId: string; levels: number }[],
  ): Promise<{ skillPoints: number }> => {
    const res = await fetchWithAuth(`${apiUrl}/skills/invest-batch`, {
      method: 'POST',
  ...
  adminUpdateConfig: async (
    data: Partial<SkillConfig>,
  ): Promise<SkillConfig> => {
    const res = await fetchWithAuth(`${apiUrl}/admin/skills/config`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
  ...
  adminDeleteEdge: async (
    fromNodeId: string,
    toNodeId: string,
  ): Promise<void> => {
    const res = await fetchWithAuth(
      `${apiUrl}/admin/skills/edges/${fromNodeId}/${toNodeId}`,
      {
        method: 'DELETE',
      },
    )
