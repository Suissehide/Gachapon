...
/** Mouse-tracked CSS vars driving <HoloOverlay /> — attach the returned ref and
 *  handlers to the card root. Only `active` triggers re-renders; positions are
 *  written straight to the DOM. */
export function useHoloPointer(): {
  rootRef: RefObject<HTMLDivElement | null>
  holoActive: boolean
  onHoloMouseMove: (e: MouseEvent<HTMLDivElement>) => void
  onHoloMouseLeave: () => void
} {
  const [holoActive, setHoloActive] = useState(false)
  const rootRef = useRef<HTMLDivElement>(null)

  const onHoloMouseMove = (e: MouseEvent<HTMLDivElement>) => {
  ...
  const onHoloMouseLeave = () => {
    setHoloActive(false)
    // Layers whose opacity rides --holo-opc (gold glare) must fade back out.
    rootRef.current?.style.setProperty('--holo-opc', '0')
  }

  return { rootRef, holoActive, onHoloMouseMove, onHoloMouseLeave }
}
      ...
      <div
        ...
        style={{
          ...
          ...(active
            ? {
                backgroundPosition:
                  'calc(var(--holo-lp, 50) * 1%) calc(var(--holo-tp, 50) * 1%)',
              }
            : {}),
        }}
      />
      ...
      <div
        ...
        style={{
          ...
          ...(active
            ? {
                backgroundPosition:
                  'calc(var(--holo-px, 50) * 1%) calc(var(--holo-py, 50) * 1%)',
