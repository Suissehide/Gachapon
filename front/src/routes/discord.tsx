      ...
      <div className="pt-32 pb-24 px-6 lg:px-10 max-w-4xl mx-auto">
        ...
        <div className="mb-12">
          ...
          <p className="text-base text-text-light leading-relaxed max-w-xl">
            Connecte ton serveur Discord à Gachapon via l'API publique.
            Tes membres pourront tirer des capsules, consulter leur collection
            et suivre le classement — sans quitter Discord.
          </p>
        ...
        <div>
          <Step n={1} title="Créer l'application Discord">
            <ol className="space-y-2 text-sm text-text-light">
              <li>1. Ouvre le <a href="https://discord.com/developers/applications" target="_blank" rel="noopener noreferrer" className="text-primary underline underline-offset-2">Discord Developer Portal</a></li>
              <li>2. Clique sur <strong className="text-foreground">New Application</strong>, donne un nom à ton bot</li>
              <li>3. Dans l'onglet <strong className="text-foreground">Bot</strong>, clique <strong className="text-foreground">Reset Token</strong> et note le token</li>
              <li>4. Active <strong className="text-foreground">Server Members Intent</strong> et <strong className="text-foreground">Message Content Intent</strong></li>
              <li>5. Dans <strong className="text-foreground">OAuth2 → URL Generator</strong>, coche <code className="text-xs bg-muted px-1 py-0.5 rounded">bot</code> + <code className="text-xs bg-muted px-1 py-0.5 rounded">applications.commands</code>, puis invite le bot sur ton serveur</li>
            </ol>
          </Step>
          ...
          <Step n={2} title="Générer ta clé API Gachapon">
            <p className="text-sm text-text-light mb-3">
              ...
              <Link to="/_authenticated/settings" className="text-primary underline underline-offset-2">
                Paramètres → Clés API
              </Link>{' '}
              et génère une nouvelle clé. Elle sera passée dans le header{' '}
              <code className="text-xs bg-muted px-1 py-0.5 rounded">X-API-Key</code> de chaque requête.
            </p>
            <div className="rounded-lg border border-amber-500/20 bg-amber-500/5 px-4 py-3 text-xs text-amber-600 dark:text-amber-400">
              ⚠ Stocke ta clé dans une variable d'environnement, jamais en clair dans le code.
            </div>
          </Step>

          <Step n={3} title="Initialiser le projet">
            <CodeBlock language="bash" code={`mkdir gachapon-bot && cd gachapon-bot
npm init -y
npm install discord.js dotenv`} />
            <p className="text-sm text-text-light mb-2">Crée un fichier <code className="text-xs bg-muted px-1 py-0.5 rounded">.env</code> :</p>
            <CodeBlock language=".env" code={`DISCORD_TOKEN=ton_token_discord
...
GACHAPON_API_URL=${baseUrl}`} />
          ...
          <Step n={4} title="Enregistrer les commandes slash">
            <p className="text-sm text-text-light mb-2">
              Crée <code className="text-xs bg-muted px-1 py-0.5 rounded">deploy-commands.js</code> pour déclarer les commandes auprès de Discord :
            </p>
            <CodeBlock language="javascript" code={`import { REST, Routes, SlashCommandBuilder } from 'discord.js'
...
console.log('Commandes enregistrées ✓')`} />
          ...
          <Step n={5} title="Commande /pull">
            <p className="text-sm text-text-light mb-2">
              La commande principale : elle appelle <code className="text-xs bg-muted px-1 py-0.5 rounded">POST /pulls</code> et affiche le résultat.
            </p>
            <CodeBlock language="javascript" code={`// handlers/pull.js
    ...
    embeds: [{
      ...
      footer: { text: 'Gachapon' },
          ...
          <Step n={6} title="Commandes /collection et /classement">
            <CodeBlock language="javascript" code={`// handlers/collection.js
...
client.login(process.env.DISCORD_TOKEN)`} />
        ...
        <div className="mt-12 rounded-xl border border-primary/20 bg-primary/5 p-6 flex items-center justify-between gap-4 flex-wrap">
          <div>
            <p className="text-sm font-semibold text-foreground mb-1">Endpoints disponibles</p>
            <p className="text-xs text-text-light">
              Consulte la documentation interactive pour voir tous les endpoints et leurs paramètres.
            </p>
          </div>
          <Link
            to="/api-docs"
